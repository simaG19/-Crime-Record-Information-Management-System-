...
if(isset($_POST['submit'])){
	...
	if(mysqli_num_rows(mysqli_query($con,$sql))>0){
		...
		if($id==''){
			...
			mysqli_query($con,"insert into user(name,username,password,email,gender,dob,station,address,phone,role,status,added_on) values('$name','$username','$password','$email','$gender','$dob','$station','$address','$phone','$role',1,'$added_on')");
		}else{
            $image_condition='';
